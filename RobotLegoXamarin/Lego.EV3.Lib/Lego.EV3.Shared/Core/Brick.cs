...
namespace Lego.Ev3.Core
{
	...
	public sealed class Brick
	{
		...
		public Brick(ICommunication comm, Func<Brick, SystemCommand> systemCommandFactory) : this(comm, systemCommandFactory, false) { }
		...
		public Brick(ICommunication comm, Func<Brick, SystemCommand> systemCommandFactory, bool alwaysSendEvents)
		{
			DirectCommand = new DirectCommand(this);
            SystemCommand = systemCommandFactory(this);// new SystemCommand(this);
        ...
        {
            return ConnectAsyncInternal(TimeSpan.FromMilliseconds(5), deviceName)
#if WINRT
			.AsAsyncAction()
#endif
            ;
        ...
        Task
#endif
        ConnectAsync(TimeSpan pollingTime, string deviceName = null)
        {
            return ConnectAsyncInternal(pollingTime, deviceName)
#if WINRT
			.AsAsyncAction()
#endif
            ;
        ...
        private async Task ConnectAsyncInternal(TimeSpan pollingTime, string deviceName = null)
		{
			_tokenSource = new CancellationTokenSource();

			await _comm.ConnectAsync(deviceName);
			...
			if(pollingTime != TimeSpan.Zero)
			{
				...
				Task t = Task.Factory.StartNew(async () =>
				{
					...
					await DirectCommand.StopMotorAsync(OutputPort.All, false);
                }, /*_tokenSource.Token*/CancellationToken.None, TaskCreationOptions.DenyChildAttach, TaskScheduler.Default);
			}
		}
   ...
   //         foreach(InputPort i in new InputPort[]{InputPort.One, InputPort.Two, InputPort.Three, InputPort.Four})//Enum.GetValues(typeof(InputPort)))
			//{
			//	Port p = Ports[i];
            ...
            foreach(InputPort i in Enum.GetValues(typeof(InputPort)))
            {
              ...
              c.ReadyPercent(p.InputPort, p.Mode, (byte)(index+10));
                ...
                Debug.WriteLine("Command InputPort");
                await SendCommandAsyncInternal(c);
                if(c.Response.Data == null)
                return;

                int type = c.Response.Data[0];
                byte mode = c.Response.Data[1];
                float siValue = BitConverter.ToSingle(c.Response.Data, 2);
                int rawValue = BitConverter.ToInt32(c.Response.Data, 6);
                byte percentValue = c.Response.Data[10];
            ...
            {
                Command c = new Command(CommandType.DirectReply, 6, 0);
                ...
                Debug.WriteLine("Command Buttons");
                await SendCommandAsyncInternal(c);
                if (c.Response.Data == null)
                    return;

                if (Buttons.Back != (c.Response.Data[0] == 1) ||
                Buttons.Left != (c.Response.Data[1] == 1) ||
                Buttons.Up != (c.Response.Data[2] == 1) ||
                Buttons.Right != (c.Response.Data[3] == 1) ||
                Buttons.Down != (c.Response.Data[4] == 1) ||
                Buttons.Enter != (c.Response.Data[5] == 1)
            )
                    changed = true;

                Buttons.Back = (c.Response.Data[0] == 1);
                Buttons.Left = (c.Response.Data[1] == 1);
                Buttons.Up = (c.Response.Data[2] == 1);
                Buttons.Right = (c.Response.Data[3] == 1);
                Buttons.Down = (c.Response.Data[4] == 1);

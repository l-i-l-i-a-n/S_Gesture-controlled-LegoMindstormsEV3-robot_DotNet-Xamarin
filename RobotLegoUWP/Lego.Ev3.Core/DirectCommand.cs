...
namespace Lego.Ev3.Core
{
	...
	public sealed class DirectCommand
	{
		...
		Task
#endif
		PlaySoundAsync(int volume, string filename, bool loop=false)
		{
			return PlaySoundAsyncInternal(volume, filename, loop)
#if WINRT
			.AsAsyncAction()
#endif
			;
...
Task<bool>
		...
		internal async Task PlaySoundAsyncInternal(int volume, string filename, bool loop = false)
		{
			Command c = new Command(CommandType.DirectNoReply);
			c.PlaySound(volume, filename, loop);
			await _brick.SendCommandAsyncInternal(c);
		}

        internal async Task StopSoundAsyncInternal()
        {
            Command c = new Command(CommandType.DirectNoReply);
            c.StopSound();
            await _brick.SendCommandAsyncInternal(c);
		...
		internal async Task<int> ReadyRawAsyncInternal(InputPort port, int mode)
		{
            ...
            await _brick.SendCommandAsyncInternal(c);
            if(c == null || c.Response == null || c.Response.Data == null)
            {
                return 0;
            }
			int temp = BitConverter.ToInt32(c.Response.Data, 0);
            return temp;
		}
